...
rl.question("What project name do you want to use? ", (answer) => {
  ...
  const repoName = "create-figma-plugin";
  const createDirCommand = `mkdir ${userProjectDir}`;
  const gitCheckoutCommand = `cd ${userProjectDir} && git clone --depth 1 https://github.com/boraaonur/create-figma-plugin`;
  ...
  fs_1.default.readdirSync(sourcePath).forEach((file) => {
    fs_1.default.renameSync(
      path_1.default.join(sourcePath, file),
      path_1.default.join(destPath, file)
    );
  });
  // Remove the empty 'create-figma-plugin' directory
  ...
  (0, utils_1.runCommand)(`cd ${userProjectDir}/plugin && npm install`);
  (0, utils_1.runCommand)(`cd ${userProjectDir}/ui && npm install`);
